...
function handleNewPosition(pos) {
  ...
  socket.emit("LocationfromClient", locationData);
  if (mapInit) {
...
function updateMapContent() {
  ...
  let otherPosOnCanvas = myMap.latLngToPixel(otherLatitude, otherLongitude);
  other.goalX = otherPosOnCanvas.x;
  other.goalY = otherPosOnCanvas.y;
}
socket.on("LocationfromClient", function (locationData) {
  otherLongitude = locationData.lng;
  otherLatitude = locationData.lat;
  if (mapInit) {
    // if map already displayed, update the point
    updateMapContent();
  }
